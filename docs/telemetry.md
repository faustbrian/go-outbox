...
exposed by `go-telemetry.Runtime`. Core and the `go-queue` adapter do not depend
on OpenTelemetry.
...
cardinality. A `go-log` logger can be passed directly through
`relay.Config.Logger` because it uses the standard `*slog.Logger` surface.
