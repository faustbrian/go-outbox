# Consumer Idempotency With go-idempotency
...
fingerprint, err := idempotency.NewFingerprint(
    "go-outbox-envelope-v1", envelope.CanonicalJSON(),
)
if err != nil {
    return err
}
...
With the PostgreSQL `go-idempotency` store, persist the business effect and
