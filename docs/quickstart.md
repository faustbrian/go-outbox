...
```sh
go get github.com/faustbrian/go-outbox@v1.0.0
```

Version 1 follows the compatibility and migration policies documented in this
repository. Pin an exact version and review the changelog before upgrading.
...
and how migrations run; `go-outbox` does not migrate at package initialization.
...
[`go-idempotency` integration guide](idempotency.md) covers canonical
