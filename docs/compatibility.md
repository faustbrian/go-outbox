...
| go-queue | `5036902eed67` | Standalone adapter race, coverage, fuzz, acceptance, error, and cancellation tests |
| go-telemetry | adapter-pinned | Runtime standard providers and propagator |
...
| `goqueue.Publisher` | synchronous `Queue` returns `nil` | Rejects an already-canceled context; pinned `go-queue` has no context parameter, so an in-flight call cannot be interrupted | A queue error uses outbox retry policy; retries performed by a job worker after queue acceptance are downstream and do not cause outbox republish |
...
outbox to retry an envelope already accepted by `go-queue`. The call can delay
