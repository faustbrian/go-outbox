...
| H-02 | High | The documented `go-migrations` integration rejected split migration filenames, so a clean production install through that runner was impossible. | Closed by the canonical source plus green real parser and concurrent-runner tests through `make migration-integration`. |
...
| L-02 | Low | Pinned `go-queue` cannot cancel an in-flight synchronous producer call. | Accepted upstream contract; deterministic test prevents cancellation after acceptance from manufacturing a duplicate. Worker network/request timeouts must bound the call. |
...
| Standalone adapters | `make adapter` | Pass: `go-queue` and telemetry race suites at 100% coverage |
| Remote workflows | PostgreSQL, CI, publisher integration, fuzz, benchmark, and security workflows for the release commit | Pass: [CI and publisher adapters](https://github.com/faustbrian/go-outbox/actions/runs/29414378391), [PostgreSQL 14-18](https://github.com/faustbrian/go-outbox/actions/runs/29414378471), [security](https://github.com/faustbrian/go-outbox/actions/runs/29414378352), [fuzzing](https://github.com/faustbrian/go-outbox/actions/runs/29414394360), and [benchmarks](https://github.com/faustbrian/go-outbox/actions/runs/29414395951) on the release tree; the pull-request-only dependency review was intentionally not applicable to the `main` push |
