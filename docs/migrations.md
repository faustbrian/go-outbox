...
`-- +migrations Down` sections accepted directly by `go-migrations`. The
...
the real `go-migrations.FSSource` and verifies identity, sections, and checksum.
Its runner contract test launches two concurrent runners and verifies one
clean owned-ledger record; run it whenever the sibling `go-migrations` checkout
changes:
...
Set `GO_MIGRATIONS_DIR=/path/to/go-migrations` when the sibling checkout is not
at `../go-migrations`. The command creates and removes its own temporary Go
