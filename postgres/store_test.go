...
func TestReplayPreservesTransactionFailures(t *testing.T) {
	...
	for name, test := range tests {
		t.Run(name, func(t *testing.T) {
			...
			store, err := newStore(test.database, StoreConfig{LeaseTokenGenerator: token})
			if err != nil {
				t.Fatalf("create store: %v", err)
			}
	...
	store, err := newStore(&faultDatabase{}, StoreConfig{LeaseTokenGenerator: func() (string, error) { return "", nil }})
	if err != nil {
...
func TestLeaseTokenAndErrorTextHelpers(t *testing.T) {
	...
	if errorText(nil) != "" || errorText(readErr) != "read failed" {
		t.Fatal("error text did not preserve expected values")
	}
}
...
func (*faultTx) Rollback(context.Context) error {
	return nil
}
