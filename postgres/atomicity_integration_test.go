...
import (
	...
	outboxrelay "github.com/faustbrian/go-outbox/relay"
...
func TestApplicationWriteAndOutboxRecordAreAtomic(t *testing.T) {
	...
	migration, err := fs.ReadFile(outboxpostgres.Migrations(), "000001_create_outbox.up.sql")
	if err != nil {
	...
	store, err := outboxpostgres.NewStore(pool, outboxpostgres.StoreConfig{MaxClaimBatch: 4})
	if err != nil {
	...
	if err := store.Retry(ctx, outboxpostgres.LeaseRef{ID: third.Envelope.ID, Token: third.LeaseToken}, retryAt, errors.New("publisher unavailable")); err != nil {
...
ANALYZE outbox_plan`); err != nil {
		t.Fatalf("seed plan table: %v", err)
	}
	claimPlan := explainPlan(t, ctx, pool, `
SELECT id
FROM outbox_plan
...
LIMIT 100`)
	...
	retentionPlan := explainPlan(t, ctx, pool, `
SELECT id
FROM outbox_plan
WHERE state = 'delivered' AND delivered_at < clock_timestamp() - interval '1 day'
ORDER BY delivered_at, id
LIMIT 100`)
	if !usesIndex(retentionPlan) || strings.Contains(retentionPlan, "Seq Scan") {
		t.Fatalf("retention plan does not use terminal index:\n%s", retentionPlan)
	}

	proveCanceledTransitionsAreAtomic(t, ctx, pool)

	downMigration, err := fs.ReadFile(outboxpostgres.Migrations(), "000001_create_outbox.down.sql")
	if err != nil {
...
func proveCanceledTransitionsAreAtomic(
	...
	store, err := outboxpostgres.NewStore(pool, outboxpostgres.StoreConfig{Table: "outbox_crash"})
	if err != nil {
	...
	if err := store.Retry(canceled, retryRef, now.Add(time.Hour), errors.New("retry")); !errors.Is(err, context.Canceled) {
	...
	if err := store.Retry(ctx, retryRef, now.Add(time.Hour), errors.New("retry")); err != nil {
