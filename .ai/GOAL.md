...
- compatible with `go-queue` publishers without depending on application code
...
- idempotency-key hooks and optional `go-idempotency` integration without false
  exactly-once claims

### PostgreSQL Store

- documented schema and migrations consumable through `go-migrations`, without
...
- first-class adapter for `go-queue`
...
- OpenTelemetry trace linkage through optional `go-telemetry` integration
- `go-log`-compatible structured diagnostics without payload disclosure
...
- `go-queue` relay example
- consumer `go-idempotency` integration guide
...
4. Implement `go-queue` adapter, operations, observability, and examples.
