...
audit of `go-outbox`, then close every gap before production release.
...
- publisher contracts, including `go-queue`, used by adapters
- transactional outbox pattern guarantees, clearly separated from package
  policy and application responsibility
- `.ai/GOAL.md`, `go-migrations` schema integration, public APIs, docs, tests,
  fuzzers,
