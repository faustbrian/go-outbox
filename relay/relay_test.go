...
func TestRunOnceSchedulesClassifiedRetry(t *testing.T) {
	...
	if !store.retried[0].availableAt.Equal(now.Add(30*time.Second)) || !errors.Is(store.retried[0].cause, publishErr) {
...
func (store *recordingStore) Retry(_ context.Context, lease postgres.LeaseRef, availableAt time.Time, cause error) error {
	store.mu.Lock()
	defer store.mu.Unlock()
	store.retried = append(store.retried, retryCall{lease: lease, availableAt: availableAt, cause: cause})
